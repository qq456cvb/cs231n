...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
  ...
  ##############################################################################
  ln_combine = np.dot(x, Wx) + np.dot(prev_h, Wh) + b
  next_h = np.tanh(ln_combine)
  cache = x, prev_h, Wx, Wh, ln_combine
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
  return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
  ...
  ##############################################################################
  x, prev_h, Wx, Wh, ln_combine = cache
  dtanh = 1 - np.square(np.tanh(ln_combine))
...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  for t in xrange(x.shape[1]):
    last_h, cache['%d'%t] = rnn_step_forward(x[:, t, :], last_h, Wx, Wh, b)
...
def rnn_backward(dh, cache):
  ...
  for t in reversed(xrange(T)):
    if t == T-1:
      ...
      ddh = dh[:,t,:] + ddprev_h
    ddx, ddprev_h, ddWx, ddWh, ddb = rnn_step_backward(ddh, cache['%d'%t])
    dx[:,t,:] = ddx
    dWx += ddWx
    dWh += ddWh
    db += ddb
